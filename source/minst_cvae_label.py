...
class CVAE(tf.keras.Model):
    ...
    def encode_label(self, labels):
        '''
        这里相当于训练一个多维语义来表示某个数字,one-hot也是其中一种表现形式。
        
        这里用两个简单的全连接层来训练，语义向量。
        '''
        return self.label_net(labels)
...
def dataLoader(cfg=Config()):
    ...
    test_label = tf.one_hot(test_label, depth=10).numpy()
    # Normalization
...
class App:
    ...
    def compute_loss(self, x, labels):
        ...
        label_vec = model.encode_label(labels)
        # 修改一下loss function，向类别均值靠拢
    ...
    def gen_plot(self, epoch, test_input):
        labels = [i % 10 for i in range(self.cfg.sample_num)]
        labels = tf.one_hot(labels, depth=10)
        label_vec = self.model.encode_label(labels)
        predictions = self.model.sample(test_input+label_vec)
...
if __name__ == '__main__':
    ...
    model, img_name = CVAE(), 'minst_cvae_label'
